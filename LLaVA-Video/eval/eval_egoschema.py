...
def train(args) -> None:
    ...
    for line in tqdm(shard_dataset):
        ...
        video_path = os.path.join(args.data_path, "videos", f"{q_uid}.mp4")
        ...
        if not pred_answer:
            pred_idx = random.choice([0, 1, 2, 3, 4])
    ...
    if global_rank == 0:
        ...
        for output in all_output:
            if output["q_uid"] in answers:
                total += 1
                if int(output["pred"]) == int(answers[output["q_uid"]]):
                    correct += 1
        ...
        for output in all_output:
            item = {}
            item["q_uid"] = output["q_uid"]
            item["answer"] = int(output["pred"])
            if output["q_uid"] not in q_uids:
                q_uids.append(output["q_uid"])
