            ...
            try:
                ...
                if 'image' in sources[0]:
                    ...
                    if suffix == 'pkl':
                        ...
                        ##### modify #####
                        start_idx, end_idx = self.list_data_dict[i]['start_idx'], self.list_data_dict[i]['end_idx']
                        video_len = int(video_info['feats'].shape[0])
                        if end_idx >= video_len:
                            end_idx = video_len - 1
                        ...
                        # elif 3600 < duration <= 7200:
                        #     step = 8
                        # elif 2048 < duration <= 4096:
                        #     step = 16

                        # step = 1
                        # image = torch.from_numpy(video_info['feats'][: end_idx, 1:])
                        image = torch.from_numpy(video_info['feats'][start_idx: end_idx, 1:])
                        image = image[0::step, :, :]

                        input_prompt = "<image>" * image.shape[0]
                        ##### modify #####
...
def train():
    ...
    ##### modify #####
    # print("##### all trainable named_parameters #####")
